...
public class SpreadSheetController {
  ...
  /**
   * A SpreadSheetControllaer cosntructor.
   *
   * @param sheet      A SpreadSheet
   * @param readable   a Readable
   * @param appendable a Appendable
   */
  ...
  /**
   * A go method that receives input from users.
   *
   * @throws IllegalStateException if the Appendable does not work correctly.
   */
      ...
      switch (userInstruction) {
        case "assign-value": //assign a value to a cell
          try {
            ...
            writeMessage("Setting cell (" + row + "," + (col - 1) + ") \n");
        ...
        case "bulk-assign":
          try {
            ...
            col2 = sc.nextInt();
            value = sc.nextDouble();
            if (sheet instanceof BetterSpreadSheet) {
              BetterSpreadSheet better = (BetterSpreadSheet) sheet;
              better.bulkAssign(value, row, col, row2, col2);
              writeMessage("Assign Value: " + value + " from Cell (" + row + ","
                      + (col - 1) + ") to" + "(" + row2 + "," + (col2 - 1) + ") \n");
            } else {
              writeMessage("Operation not supported");
            }
          } catch (IllegalArgumentException e) {
            writeMessage("Error: " + e.getMessage() + System.lineSeparator());
          }
